...
import {updateAdminCategory} from '../../../store/actions/adminActions';
import { FaSpinner } from 'react-icons/fa';
import toast from 'react-hot-toast';
const UpdateCategoryForm = ({setOpen,selectedCategory}) => {
  const {isLoading,errorMessage}=useSelector((state)=>state.errors);
  const [categoryName,setCategoryName]=React.useState(selectedCategory?.categoryName || "");
 ...
 const [errors, setErrors] = React.useState({});
  ...
  const handleCategoryNameUpdate = (e) => {
    ...
    if (validateForm()) {
      ...
      dispatch(updateAdminCategory(sendData, toast, setOpen));
    }
  }

  return (
    <div className='py-4 relative h-full'>
      <form className='space-y-4' onSubmit={handleCategoryNameUpdate}>
